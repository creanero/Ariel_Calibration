...
def fix_matched_files(data, stepsize = 5):
    top_wavelength = data["Wavelength"][0]
    matched_count = 0
    for row in range(len(data)):
        if data["Wavelength"][row] == top_wavelength:
            matched_count = matched_count + 1

    if matched_count > 1:
        new_start_wavelength = top_wavelength - (stepsize * (matched_count - 1))
        for row in range(matched_count):
            data["Wavelength"][row] = new_start_wavelength + (stepsize * row)
